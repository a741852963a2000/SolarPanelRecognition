...
class HPF_MS_NewVGG16():
	def __init__(self, weightDir='./weight'):
		self.name = 'HPF_MS_NewVGG16'
	...
	def get_fusion_conv_weight(self, k=3, i=6, f=4):
		...
		weights[:, :, 2, :] = np.stack([self.get_conv_ident(k), self.get_conv_zero(k), self.get_conv_zero(k), self.get_conv_zero(k)], axis=-1)
		weights[:, :, 3, :] = np.stack([self.get_conv_zero(k), self.get_conv_ident(k), self.get_conv_zero(k), self.get_conv_zero(k)], axis=-1)
		weights[:, :, 4, :] = np.stack([self.get_conv_zero(k), self.get_conv_zero(k), self.get_conv_ident(k), self.get_conv_zero(k)], axis=-1)
		weights[:, :, 5, :] = np.stack([self.get_conv_zero(k), self.get_conv_zero(k), self.get_conv_zero(k), self.get_conv_ident(k)], axis=-1)
		return weights
	...
	def createModel(self):
		...
		MS_input = Input(shape=(15, 15, 4), name='MS_input')
		MS_resize = Lambda(lambda t:tf.image.resize(t, [60, 60], method=tf.image.ResizeMethod.BICUBIC), name="MS_resize")(MS_input)
		
		concat = Concatenate()([PAN_input, PAN_input, MS_resize])
		# fusion conv
		k = 3
		fusion_conv = Conv2D(filters=4, kernel_size=k, padding='same')
